var BaseGraph = require ('./BaseGraph');
var Point3d   = require('../Point3d');
var support   = require ('./Support');


function PointGraph() {
  //BaseGraph.call(this);
}
support.derive(PointGraph, BaseGraph);


PointGraph.prototype.getDotSize = function(graph3d) {
  return graph3d.frame.clientWidth * graph3d.dotSizeRatio;
};


PointGraph.prototype._calcRadius = function(graph3d, point, size) {
  var radius;
  if (graph3d.showPerspective) {
    radius = size / -point.trans.z;
  }
  else {
    radius = size * - (graph3d.eye.z / graph3d.camera.getArmLength());
  }
  if (radius < 0) {
    radius = 0;
  }

  return radius;
}


PointGraph.prototype.getColor = function(graph3d, point) {
  // Calculate Hue from the current value.
  // At zMin the hue is 240, at zMax the hue is 0
  var tmpHue = (point.point.z - graph3d.zMin) * graph3d.scale.z;
  var hue    = (1 - tmpHue / graph3d.verticalRatio) * 240;

  return {
    color      : graph3d._hsv2rgb(hue, 1, 1),
    borderColor: graph3d._hsv2rgb(hue, 1, 0.8)
  };
};


PointGraph.prototype.drawCircle = function(graph3d, ctx, point, size) {
  var radius = this._calcRadius(graph3d, point, size);
  var colors = this.getColor(graph3d, point);

  // draw the circle
  ctx.lineWidth   = graph3d._getStrokeWidth(point);
  ctx.strokeStyle = colors.borderColor;
  ctx.fillStyle   = colors.color;
  ctx.beginPath();
  ctx.arc(point.screen.x, point.screen.y, radius, 0, Math.PI*2, true);
  ctx.fill();
  ctx.stroke();
};


/**
 * Draw a datapoint as a colored circle.
 */
PointGraph.prototype.redrawPoint = function(graph3d, ctx, point) {
  var dotSize = this.getDotSize(graph3d);
  this.drawCircle(graph3d, ctx, point, dotSize);
};


module.exports = PointGraph;
