...
var DynamicResize = require('./components/DynamicResize.js').default;
...
class Canvas {
  ...
  setOptions(options) {
    ...
    if (this.options.autoResize === true) {
      ...
      if (this.resizeFunction !== undefined) {  // paranoia, prob not necessary
  ...
  /**
   * Set a new size for the network or resize due to changes in view
   *
   * --------------------------------------------------------------------------------------
   * ## NOTES
   *
   * 1. The problem here is that this method is used for two different but related purposes:
   *
   *  1. Reset the width/height options for the canvas.
   *  2. Recalculate the actual width/height based on current sizing.
   *
   * These are really not the same thing.
   *
   *
   * 2. Updating the pixel ratio here is rather inconsistent; this is the ONLY place in the code
   ...
   * --------------------------------------------------------------------------------------
   *
   * @param {string} [width]   Width in pixels or percentage (for example '800px' or '50%')
   * @param {string} [height]  Height in pixels or percentage  (for example '400px' or '30%')
   * @returns {boolean} true if size recalculated (even if result the same)
   */
  setSize(width, height) {
    var optionsChanged = false;  // If true, need resize
    ...
    // On the first call, frame width and height may not be set. Force this.
    if (this.frame.style.width === "" || this.frame.style.height == "") {
      optionsChanged = true;
    }
    ...
    // update the pixel ratio - See Note 2 in header comment.
    let previousRatio = this.pixelRatio; // we cache this because the camera state storage needs the old value
    this._setPixelRatio();

    let needWidthResize  = this.options.width.indexOf('%') !== -1;
    let needHeightResize = this.options.height.indexOf('%') !== -1;
    let didRecalculate = false;

    if (optionsChanged || needWidthResize || needHeightResize) {
      this._getCameraState(previousRatio);

      let [newWidth, newHeight] = DynamicResize.determineSize(this.frame);
      let newWidthPx  = this.options.width;
      if (needWidthResize) {
        newWidthPx = Math.round(newWidth*parseInt(this.options.width)/100) + 'px';
      }
      let newHeightPx = this.options.height;
      if (needHeightResize) {
        newHeightPx = Math.round(newHeight*parseInt(this.options.height)/100) + 'px';
      }


      // The canvas is contained in a div-element, which is added at runtime
      // This element is resized, instead of the canvas.
      this.frame.style.width = newWidthPx;
      this.frame.style.height = newHeightPx;
    ...
    let sizeChanged = (oldWidth !== this.frame.canvas.width) || (oldHeight !== this.frame.canvas.height);
